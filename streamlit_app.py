...
<style>
    ...
    .store-badges img { background: transparent !important; border: none !important; }
    
    .apple-badge { height: 42px !important; width: auto !important; margin-top: 8px !important; }
    .google-badge { height: 60px !important; width: auto !important; }
</style>
...
def clean_username(text: str) -> str:
    """Kullanıcı girdisini temizler, URL varsa username'i çıkarır."""
    if not text:
        return ""
...
def clean_number(value, default=0.0) -> float:
    """Gelen metrikleri güvenli bir şekilde ondalık sayıya çevirir."""
    try:
...
def fetch_apify_instagram_data(username: str, max_posts: int = 24):
    """Apify altyapısı kullanarak gerçek zamanlı veri çeker."""
    ...
    try:
        ...
        # Verinin hazır olmasını bekleyen Polling yapısı
        for _ in range(30):
            time.sleep(2)
...
def fetch_tiktok_data_simulated(username: str):
    """TikTok API entegrasyonu tamamlanana kadar çalışan simülasyon."""
    time.sleep(1.5)
    return {
...
def draw_aqs_gauge(score: int):
    """AQS Skorunu gösteren hız kadranı grafiği oluşturur."""
...
def draw_dna_radar(dna_scores: list):
    """Profilin karakteristik özelliklerini 5 eksenli radarda gösterir."""
...
def draw_trend_line(eng_list: list):
    """Son gönderilerin etkileşim trendini alan grafiği olarak çizer."""
    df = pd.DataFrame({
        "Gönderi": [f"Post {i+1}" for i in range(len(eng_list))], 
        "Etkileşim": reversed(eng_list)
    })
...
def run_all_algorithms(followers: int, posts: list, platform: str = "• Instagram", budget: float = 50000.0, username: str = ""):
    ...
    # Platform Spesifik Algoritmalar
    if "TikTok" in platform:
        ...
        if len(posts) > 1 and avg_views > 0:
            cv_val = float(np.std(views)) / avg_views
        else:
            cv_val = 1.0
        ...
        com_anom = 0.0
        if comment_ratio < 0.008:
            com_anom = 0.50
        elif comment_ratio < 0.012:
            com_anom = 0.25
        ...
        var_anom = 0.0
        if cv_val < 0.28 and len(posts) > 4:
            var_anom = 0.40
        elif cv_val > 1.2:
            var_anom = 0.30
    ...
    comment_score = 40.0 * (1 - com_anom)
    aqs = int(np.clip(er_score + comment_score + stability_score, 10, 99))
    
    if bot_pct > 30.0: 
        aqs = int(aqs * 0.4)

    # DNA ve Yönetici Özeti Hazırlığı
    dna_er = min(100, (er / benchmark_er) * 100)
    dna_reach = min(100, (est_reach / followers) * 100)
    dna = [dna_er, stability_score * 5, auth_pct, dna_reach, 85]
    ...
    if bot_pct > 20:
        ai_sum += f"Ancak etkileşim anormallikleri sebebiyle %{bot_pct:.1f} oranında manipülasyon (bot) tespit edilmiştir. "
    else:
    ...
    # Sabit Demografi (API Sağlamadığında)
...
<div class="store-footer">
    ...
    <div class="store-badges">
        <a href="#" target="_blank">
            <img class="apple-badge" src="https://upload.wikimedia.org/wikipedia/commons/3/3c/Download_on_the_App_Store_Badge.svg" alt="App Store">
        </a>
        <a href="#" target="_blank">
            <img class="google-badge" src="https://upload.wikimedia.org/wikipedia/commons/7/78/Google_Play_Store_badge_EN.svg" alt="Google Play">
        </a>
    </div>
</div>
""", unsafe_allow_html=True)
