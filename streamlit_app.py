...
<style>
    ...
    .hero-title { 
        ...
        animation: gradient-glow 5s linear infinite !important; margin: 0 !important; letter-spacing: -2px !important; 
    }
    ...
    .stTextInput input:focus, .stNumberInput input:focus { border-color: #ffffff !important; box-shadow: 0 0 0 2px rgba(255, 255, 255, 0.2) !important; }

    button[kind="primary"] { 
        background-color: #ffffff !important; border: 2px solid #ffffff !important; border-radius: 10px !important; 
        padding: 15px 30px !important; width: 100% !important; max-width: 400px !important; display: block !important;
        margin: 25px auto 0 auto !important; transition: transform 0.2s ease !important;
    }
    button[kind="primary"] * { color: #000000 !important; font-weight: 900 !important; font-size: 1.1rem !important; letter-spacing: 1px !important; }
    button[kind="primary"]:hover { transform: scale(1.03) !important; box-shadow: 0 10px 25px rgba(255, 255, 255, 0.3) !important; }
    ...
    .stCheckbox label span { color: #94a3b8 !important; font-weight: 700 !important; }
</style>
...
def generate_dynamic_override_data(followers, avg_views, avg_likes, avg_comments):
    posts = []
    for i in range(15):
        v = int(avg_views * np.random.uniform(0.85, 1.15))
        l = int(avg_likes * np.random.uniform(0.85, 1.15))
        c = int(avg_comments * np.random.uniform(0.85, 1.15))
        posts.append({"viewsCount": v, "likesCount": l, "commentsCount": c, "caption": "FluenceAI Override Data #kurumsal"})
...
def run_all_algorithms(followers: int, posts: list, platform: str = "• Instagram", budget: float = 50000.0, username: str = ""):
    ...
    if "YouTube" in platform:
        ...
        var_anom = 0.60 if cv_val < 0.05 and len(posts) > 2 else 0.0
        est_reach = avg_views
        ...
        cv_val = float(np.std(views)) / max(avg_views, 1) if len(posts)>1 and avg_views>0 else 1.0
    ...
    word_counts = collections.Counter([w for w in re.findall(r'\b[a-zçğıöşü]{4,}\b', all_captions) if w not in stopwords]).most_common(5)
...
with c_m:
    plat = st.radio(label="", options=["• Instagram", "• TikTok", "• YouTube"], horizontal=True, label_visibility="collapsed")
    u_inp = st.text_input(label="", placeholder=f"{plat.replace('• ', '')} Hedef Profil (Örn: leyakirsan)")
    budget_inp = st.number_input(label="Planlanan Kampanya Bütçesi (₺)", min_value=1000, value=50000, step=5000)
    
    st.markdown("<br>", unsafe_allow_html=True)
    use_override = st.checkbox("API Limitlerine Takılmamak İçin Verileri Manuel Girin")
    
    if use_override:
        st.info("💡 Manuel girdiğiniz ortalama değerlere sistem tarafından %15 organik varyans eklenerek profilin detaylı analizi anında oluşturulur.")
        col1, col2, col3, col4 = st.columns(4)
        m_foll = col1.number_input("Takipçi/Abone", min_value=1, value=20000)
        m_view = col2.number_input("Ort. İzlenme", min_value=1, value=150000)
        m_like = col3.number_input("Ort. Beğeni", min_value=0, value=15000)
        m_comm = col4.number_input("Ort. Yorum", min_value=0, value=500)
    else:
        m_foll, m_view, m_like, m_comm = 0, 0, 0, 0
