...
<style>
    ...
    .stTextInput input, .stNumberInput input, .stTextArea textarea { background-color: rgba(15, 23, 42, 0.6) !important; border: 1px solid #334155 !important; border-radius: 8px !important; font-weight: 600 !important; padding: 14px 16px !important; font-size: 1rem !important; color: #f8fafc !important; backdrop-filter: blur(10px); }
...
# ---------------------------------------------------------
# 3. YARDIMCI FONKSİYONLAR VE VERİ MOTORLARI
# ---------------------------------------------------------
...
def fetch_apify_instagram_data(username, max_posts=24):
    ...
    bar = progress_container.progress(0, text="• Apify motoru başlatılıyor (Instagram)...")
...
# TIKTOK İÇİN ÖZEL VERİ SİMÜLATÖRÜ (Gerçek API bağlanana kadar güvenli test)
def fetch_tiktok_data_simulated(username):
    time.sleep(2)
    return {
        "followersCount": 450000,
        "latestPosts": [
            {"likesCount": 8500, "commentsCount": 150, "viewsCount": 1500000, "caption": "Trend denemesi #fyp", "type": "Video"}, # İzlenme botu şüphesi
            {"likesCount": 92000, "commentsCount": 1200, "viewsCount": 950000, "caption": "Günlük vlog", "type": "Video"},
            {"likesCount": 88000, "commentsCount": 1100, "viewsCount": 890000, "caption": "Dans @marka", "type": "Video"},
        ]
    }
...
# ---------------------------------------------------------
# 4. OMNICHANNEL ALGORİTMA MOTORU (IG & TIKTOK)
# ---------------------------------------------------------
def run_all_algorithms(followers, posts, platform="• Instagram", budget=0.0, username=""):
    ...
    if "TikTok" in platform:
        ...
        # TikTok ER = Eng / Views (çünkü TikTok izlenme odaklıdır)
        er = (total_eng / max(avg_views, 1.0) * 100.0) if avg_views > 0 else (total_eng / max(followers, 1.0) * 100.0)
        
        # 1. İzlenme - Beğeni Oranı (LVR) Kuralı
        lvr = avg_likes / max(avg_views, 1.0)
        comment_anomaly = 0.0
        if lvr < 0.02: comment_anomaly = 0.60 # View bot: Çok izlenme, az beğeni
        elif lvr > 0.40: comment_anomaly = 0.50 # Like bot: Az izlenme, çok beğeni
        
        # 2. Ters Varyans Kuralı (TikTok'ta stabilite sahtekarlıktır)
        variance_anomaly = 0.0
        if len(posts) > 1 and avg_views > 0:
            views_cv = float(np.std(views)) / avg_views
            if views_cv < 0.15 and len(posts) > 2: 
                variance_anomaly = 0.60 # Her video aynı izlenmiş = Paket izlenme
        else:
            views_cv = 1.0
            
        cv_val = views_cv # UI'da göstermek için
            
        # 3. Viralite Skoru
        viral_score = avg_views / max(followers, 1.0)
        
        calculated_bot = 10.0 + (comment_anomaly * 100.0) + (variance_anomaly * 100.0)
        ...
        aqs_score = int(np.clip(er_score + 60.0 - (bot_pct*0.5), 10, 99))
        
        ai_summary = f"TikTok algoritma standartlarına göre @{username} denetlendi. "
        if viral_score > 1.2: ai_summary += "Profil bir 'Keşfet (FYP) Canavarı', kendi takipçi havuzunun ötesine geçebiliyor. "
        if bot_pct > 20: ai_summary += "Ancak İzlenme/Beğeni (LVR) testlerinde suni müdahale (View/Like bot) tespit edildi. "
        else: ai_summary += "Kitle davranışı TikTok ekosistemi için oldukça organik. "
        
        est_reach = avg_views # TikTok'ta erişim = ortalama izlenmedir
        fraud_title1 = "İzlenme - Beğeni Oranı (LVR)"
        fraud_desc1 = f"Oran (%{(lvr*100):.2f}) mantıksız. İzlenme botu (View Bot) şüphesi." if lvr < 0.02 else "Reaksiyon doğal."
        fraud_title2 = "Ters Varyans (İzlenme Stabilitesi)"
        fraud_desc2 = "İzlenmeler fazla sabit. Paket otomatik izlenme atılmış." if views_cv < 0.15 else "TikTok doğasına uygun dalgalanma."

    # ------------------ INSTAGRAM ALGORİTMASI ------------------
    else:
        ...
        if len(posts) > 1: cv_val = float(np.std([(l+c)/max(followers, 1)*100 for l, c in zip(likes, comments)])) / er if er > 0 else 1.0 
        else: cv_val = 1.0
            
        stability_score = max(0.0, 20.0 * (1.0 - min(cv_val, 1.0)))
        variance_anomaly = 0.40 if (cv_val < 0.28 and len(posts) > 4) else (0.30 if cv_val > 1.2 else 0.00)
        ...
        aqs_score = int(np.clip(er_score + (40.0 if comment_ratio >= 0.015 else (comment_ratio / 0.015) * 40.0) + stability_score, 10, 99))
        if bot_pct > 30.0: aqs_score = int(aqs_score * 0.4)

        ai_summary = f"Instagram algoritma kurallarına göre @{username} denetlendi. "
        if bot_pct > 20: ai_summary += "Ciddi bir manipülasyon/bot riski mevcuttur. Dışarıdan suni müdahaleler tespit edilmiştir. "
        else: ai_summary += "Profilin bot ve manipülasyon riski (%{bot_pct:.1f}) oldukça güvenilir seviyededir. "

        visibility_multiplier = 3.5 if er > 2.0 else 2.5
        est_reach = min(int(followers * (er / 100.0) * visibility_multiplier), followers)
        
        fraud_title1 = "Yorum/Beğeni Reaksiyon Dengesi"
        fraud_desc1 = "Denge mantıksız. Panel/Bot kullanılmış." if comment_ratio < 0.008 else "Denge organik."
        fraud_title2 = "İstatistiksel Varyans (CV)"
        fraud_desc2 = "Etkileşim stabilitesi suni. Bot paketi şüphesi." if cv_val < 0.28 else "Doğal dalgalanma."

    # ORTAK METRİKLER (CPE, CPM)
    ...
    for p in posts:
        ...
        all_captions += " " + cap
        if any(kw in cap for kw in ["#reklam", "#işbirliği", "işbirliği", "fyp"]): collab_count += 1
    ...
    return {
        "followers": followers, "er": er, "total_eng": total_eng, "aqs_score": aqs_score, "cv_value": cv_val,
        ...
        "ai_summary": ai_summary, "collab_ratio": collab_ratio, "top_sectors": top_sectors, "username": username, "platform": platform,
        "fraud_title1": fraud_title1, "fraud_desc1": fraud_desc1, "fraud_title2": fraud_title2, "fraud_desc2": fraud_desc2
    }

# ---------------------------------------------------------
# 5. UYGULAMA PANELİ (DİREKT GİRİŞ)
# ---------------------------------------------------------
st.markdown("""
...
with tab_rep:
    ...
    with c_m:
        plat = st.radio("Platform Seçimi", ["• Instagram", "• TikTok"], horizontal=True)
        u_inp = st.text_input(f"{plat.replace('• ', '')} Profil Bağlantısı veya Adı", placeholder="Örn: leyakirsan")
    ...
    if b_run and u_inp:
        ...
        if plat == "• Instagram":
            ...
            with st.spinner(f"• @{r_usr} TikTok algoritması ile inceleniyor..."):
                p_dat = fetch_tiktok_data_simulated(r_usr) # TikTok için API eklenene kadar simülasyon
                
        if p_dat and "latestPosts" in p_dat:
            f_count = int(clean_number(p_dat.get("followersCount", p_dat.get("followers", 0)), 1))
            m_r = run_all_algorithms(f_count, p_dat.get("latestPosts", []), platform=plat, username=r_usr)
            ...
            <div class="exec-summary">
                <div><h2 style="margin:0;">@{r_usr}</h2><p style="color:#94a3b8;margin:0;">{f_count:,} Takipçi ({plat.replace('• ', '')})</p></div>
            ...
            c3.markdown(f"<div class='metric-card'><div class='metric-title'>Tahmini Erişim</div><div class='metric-value'>{int(m_r['est_reach']):,}</div></div>", unsafe_allow_html=True)
            ...
            st.markdown(f"<br><h4 style='color:#f8fafc; font-weight:800; border-bottom:1px solid #1e293b; padding-bottom:10px;'>• {plat.upper()} ANTI-FRAUD KARNESİ</h4>", unsafe_allow_html=True)
            
            c1_color = "#ef4444" if "AĞIR" in m_r['fraud_desc1'] else ("#f59e0b" if "ŞÜPHELİ" in m_r['fraud_desc1'] else "#10b981")
            st.markdown(f"<div class='fraud-box' style='border-left-color: {c1_color};'><div class='fraud-icon' style='color:{c1_color};'>•</div><div class='fraud-content'><h5>{m_r['fraud_title1']}</h5><p>{m_r['fraud_desc1']}</p></div></div>", unsafe_allow_html=True)

            c2_color = "#ef4444" if "AĞIR" in m_r['fraud_desc2'] or "Paket" in m_r['fraud_desc2'] else ("#f59e0b" if "ŞÜPHELİ" in m_r['fraud_desc2'] else "#10b981")
            st.markdown(f"<div class='fraud-box' style='border-left-color: {c2_color};'><div class='fraud-icon' style='color:{c2_color};'>•</div><div class='fraud-content'><h5>{m_r['fraud_title2']}</h5><p>{m_r['fraud_desc2']}</p></div></div>", unsafe_allow_html=True)
        
        else:
            st.error("• Veri çekilemedi. Bağlantıyı kontrol edin veya API kotanızı doğrulayın.")
        ...
        with c_w1:
            ...
            if d['top_mentions']:
                for mention, count in d['top_mentions']:
                    st.markdown(f"<div style='background:rgba(15,23,42,0.5); padding:10px; border-radius:8px; margin-bottom:8px; border:1px solid #1e293b; display:flex; justify-content:space-between;'><span style='color:#3b82f6;'>@{mention}</span><span style='color:#94a3b8;'>{count} Kez</span></div>", unsafe_allow_html=True)
            else:
                st.markdown("<p style='color:#64748b;'>Veri Yok</p>", unsafe_allow_html=True)
            st.markdown("</div>", unsafe_allow_html=True)
        with c_w2:
...
with tab_fin:
    ...
    with c_f:
        ...
        if st.button("FİZİBİLİTE HESAPLA"):
            with st.spinner("Finansal veri hesaplanıyor..."):
