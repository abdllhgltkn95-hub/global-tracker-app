...
# ---------------------------------------------------------
st.set_page_config(
    page_title="FluenceAI | Executive Intelligence",
    page_icon="•",
    layout="wide",
    initial_sidebar_state="collapsed",
)

# Aynı ücretsiz Apify anahtarını tüm platformlar için kullanıyoruz
...
# ---------------------------------------------------------
# 2. CSS STİLLERİ (FLUENCE AI TEMA & PITCH BLACK)
# ---------------------------------------------------------
st.markdown("""
<style>
    /* SİMSİYAH ARKA PLAN */
...
# ---------------------------------------------------------
# 3. YARDIMCI VE API FONKSİYONLARI (TIKTOK CANLI API EKLENDİ)
# ---------------------------------------------------------
...
def fetch_apify_tiktok_data(username: str, max_posts: int = 15):
    """TikTok için gerçek zamanlı Apify Scraper Motoru ve Çevirmeni."""
    run_url = f"https://api.apify.com/v2/acts/apify~tiktok-scraper/runs?token={APIFY_TOKEN}"
    payload = {"profiles": [username], "resultsPerPage": max_posts}
    ...
    try:
        ...
        # Veri Çevirmeni (TikTok JSON'ını bizim algoritmaya uyarlar)
        followers = raw_items[0].get("authorMeta", {}).get("fans", 1)
        posts = []
        for item in raw_items:
            likes = item.get("diggCount") or item.get("stats", {}).get("diggCount", 0)
            comments = item.get("commentCount") or item.get("stats", {}).get("commentCount", 0)
...
def fetch_youtube_data_simulated(username: str):
    """YouTube API kiralayana kadar sistemde stabiliteyi koruyan simülasyon."""
    time.sleep(1.8)
    return {
...
def draw_dna_radar(dna_scores: list):
    ...
    fig.add_trace(go.Scatterpolar(
        ...
        fillcolor='rgba(255, 255, 255, 0.1)', line=dict(color='#ffffff', width=2)
    ))
...
def draw_bar(x_data: list, y_data: list, title: str, color: str):
    ...
    fig.update_layout(
        title=dict(text=title, font=dict(color="#ffffff", size=16), x=0.5, xanchor='center'), 
        paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)", font=dict(color="#94a3b8"), 
        height=300, margin=dict(l=10, r=10, t=40, b=10), xaxis_title=None, yaxis_title=None, 
...
def run_all_algorithms(followers: int, posts: list, platform: str = "• Instagram", budget: float = 50000.0, username: str = ""):
    ...
    if "YouTube" in platform:
        ...
        if avg_views > 0: 
            er = (total_eng / avg_views) * 100.0
            
        lvr = np.mean(likes) / max(avg_views, 1.0)
        
        if lvr < 0.01: com_anom = 0.60
        elif lvr > 0.15: com_anom = 0.50
        else: com_anom = 0.0
            
        if len(posts) > 1 and avg_views > 0: cv_val = float(np.std(views)) / avg_views
        else: cv_val = 1.0
            
        if cv_val < 0.05 and len(posts) > 2: var_anom = 0.60
        else: var_anom = 0.0
        ...
        if avg_views > 0: 
            er = (total_eng / max(avg_views, 1.0)) * 100.0
            
        lvr = np.mean(likes) / max(avg_views, 1.0)
        
        if lvr < 0.02: com_anom = 0.60
        elif lvr > 0.40: com_anom = 0.50
        else: com_anom = 0.0
            
        if len(posts) > 1 and avg_views > 0: cv_val = float(np.std(views)) / avg_views
        else: cv_val = 1.0
            
        if cv_val < 0.15 and len(posts) > 2: var_anom = 0.60
        else: var_anom = 0.0
        ...
        if comment_ratio < 0.008: com_anom = 0.50
        elif comment_ratio < 0.012: com_anom = 0.25
        elif comment_ratio > 0.15: com_anom = 0.30
        else: com_anom = 0.0
            
        if len(posts) > 1:
            cv_val = float(np.std([(l + c) / max(followers, 1) * 100 for l, c in zip(likes, comments)])) / max(er, 0.1)
        else:
            cv_val = 1.0
            
        if cv_val < 0.28 and len(posts) > 4: var_anom = 0.40
        elif cv_val > 1.2: var_anom = 0.30
        else: var_anom = 0.0
    ...
    if bot_pct > 20: 
        ai_sum += f"Etkileşim anormallikleri sebebiyle %{bot_pct:.1f} oranında manipülasyon (bot) tespit edilmiştir. "
    else: 
        ai_sum += "Profil davranışları platform doğasına uygundur, suni müdahale izine rastlanmamıştır. "
    ...
    for p in posts:
        ...
        all_captions += " " + cap
        found_mentions = re.findall(r'@([a-zA-Z0-9_.]+)', cap)
    ...
    if com_anom > 0: f1_desc = "Suni reaksiyon tespit edildi."
    else: f1_desc = "Doğal reaksiyon akışı."
        
    if var_anom > 0: f2_desc = "Paket hizmet şüphesi."
    else: f2_desc = "İstikrarlı büyüme."
        
    if "YouTube" in platform:
        if com_anom > 0: f1_desc = "Mantıksız LVR (Beğeni/İzlenme) oranı. Bot şüphesi."
        else: f1_desc = "Doğal LVR oranı."
            
        if var_anom > 0: f2_desc = "Aşırı stabil izlenmeler. Paket view bot şüphesi."
        else: f2_desc = "YouTube SEO/Algoritma dalgalanması organik."
...
<div class='hero-container'>
    <h1 class='hero-title'>FluenceAI</h1>
...
with c_m:
    ...
    u_inp = st.text_input(
        label="", 
        placeholder=f"{plat.replace('• ', '')} Hedef Profil (Örn: leyakirsan)"
    )
    
    budget_inp = st.number_input(
        label="Planlanan Kampanya Bütçesi (₺)", 
        min_value=1000, 
        value=50000, 
        step=5000
    )
...
if b_run and u_inp:
    ...
    with st.spinner(f"• Derin veri madenciliği ve {plat.replace('• ', '')} motoru çalışıyor... Lütfen bekleyin."):
        ...
        if plat == "• Instagram": 
            ...
            p_dat = fetch_apify_tiktok_data(r_usr) # ARTIK GERÇEK API ÇAĞRILIYOR
        else: 
        ...
        if p_dat and "latestPosts" in p_dat:
            ...
            <div class='exec-summary'>
                <div>
                    ...
                    <p style='color:#94a3b8;margin:0;font-size:1.1rem;'>
                        {m_r['followers']:,} {follower_label} ({plat.replace('• ','')})
                    </p>
                </div>
                <div>
                    <span class='badge-status' style='background:{b_clr}; color:#000000; border: none;'>
                        • {b_text}
                    </span>
                </div>
            </div>
            <div class='ai-summary-box'>
                <b>🧠 AI Strateji Uzmanı:</b><br>{m_r['ai_sum']}
            </div>
            """, unsafe_allow_html=True)
            ...
            with v3: 
                st.plotly_chart(draw_donut(["Organik Kitle", "Sentetik/Bot"], [m_r['auth_pct'], m_r['bot_pct']], "Kitle Sağlığı", ["#ffffff", "#ef4444"]), use_container_width=True)
            ...
            with d1: 
                st.plotly_chart(draw_bar(list(m_r['age'].keys()), list(m_r['age'].values()), "Yaş Dağılımı", "#ffffff"), use_container_width=True)
            with d2: 
                st.plotly_chart(draw_donut(list(m_r['gender'].keys()), list(m_r['gender'].values()), "Cinsiyet", ["#ec4899", "#3b82f6"]), use_container_width=True)
            with d3:
                if m_r['words']:
            ...
            <div class='metric-card'>
                <div class='metric-title'>Tahmini Erişim</div>
                <div class='metric-value'>{int(m_r['est_reach']):,}</div>
                <div class='metric-sub'>Kişi Başı</div>
            </div>
            """, unsafe_allow_html=True)
            
            f2.markdown(f"""
            <div class='metric-card'>
                <div class='metric-title'>CPE (Etkileşim Başı)</div>
                <div class='metric-value'>₺{m_r['cpe']:.2f}</div>
                <div class='metric-sub'>Maliyet</div>
            </div>
            """, unsafe_allow_html=True)
            
            f3.markdown(f"""
            <div class='metric-card'>
                <div class='metric-title'>CPM (1000 Gösterim)</div>
                <div class='metric-value'>₺{m_r['cpm']:.2f}</div>
                <div class='metric-sub'>Maliyet</div>
            </div>
            """, unsafe_allow_html=True)
...
# ---------------------------------------------------------
# 7. MAĞAZA LOGOLARI VE FLUENCE AI COPYRIGHT FOOTER
# ---------------------------------------------------------
st.markdown("""
<div style="text-align: center; padding: 50px 0 40px 0; margin-top: 60px; border-top: 1px solid rgba(255,255,255,0.05);">
    ...
    <div style="margin-top: 30px; font-size: 0.85rem; color: #475569; letter-spacing: 2px; font-weight: 700;">
        © 2026 FLUENCE AI. TÜM HAKLARI SAKLIDIR.
    </div>
</div>
""", unsafe_allow_html=True)
