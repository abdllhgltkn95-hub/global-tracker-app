...
def draw_aqs_gauge(score: int):
    fig = go.Figure(go.Indicator(
        ...
        value = score,
        title = {
            'text': "KİTLE KALİTE SKORU (AQS)", 
            'font': {'color': '#94a3b8', 'size': 14}
        },
        number = {
            'font': {'color': '#ffffff', 'size': 50, 'weight': 'bold'}
        },
        gauge = {
...
def run_all_algorithms(followers: int, posts: list, platform: str = "• Instagram", budget: float = 50000.0, username: str = ""):
    ...
    if "YouTube" in platform:
        ...
        if cv_val < 0.05 and len(posts) > 2:
            var_anom = 0.60
        else:
            var_anom = 0.0
        ...
        if cv_val < 0.15 and len(posts) > 2:
            var_anom = 0.60
        else:
            var_anom = 0.0
    ...
    dna = [
        min(100, (er / benchmark_er) * 100), 
        stability_score * 5, 
        auth_pct, 
        min(100, viral_reach * 100), 
        85
    ]
    ...
    if com_anom > 0:
        f1_desc = "Suni reaksiyon tespit edildi."
    else:
        f1_desc = "Doğal reaksiyon akışı."
        
    if var_anom > 0:
        f2_desc = "Paket hizmet şüphesi."
    else:
        f2_desc = "İstikrarlı büyüme."
        
    if "YouTube" in platform:
        if com_anom > 0:
            f1_desc = "Mantıksız LVR (Beğeni/İzlenme) oranı. Bot şüphesi."
        else:
            f1_desc = "Doğal LVR oranı."
            
        if var_anom > 0:
            f2_desc = "Aşırı stabil izlenmeler. Paket view bot şüphesi."
        else:
            f2_desc = "YouTube SEO/Algoritma dalgalanması organik."
